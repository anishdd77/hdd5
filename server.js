...
setInterval(() => {
  http.get(`http://profile5.glitch.me/`);
}, 280000);
...
// Commands Here
let cmds = {
  profile: { cmd: 'profile', a: ['prof'] },
  setinfo: { cmd: 'setinfo', a: ['معلوماتي تعديل'] },
  rep: { cmd: 'rep', a: ['اعجاب'] },
  credits: { cmd: 'credits', a: ['c'] },
  daily: { cmd: 'daily', a: ['d'] },
  transfer: { cmd: 'trans' },
  add: { cmd: 'add' },
  buy: { cmd: 'buy' },
  set: { cmd: 'set', a: ['خلفيه'] },
  preview: { cmd: 'preview', a: ['تجربه'] },
  mywalls: { cmd: 'mywalls' }
}
...
msg.send(` :credit_card: | Transfer Receipt \`\`\`You have received ${args[0]} from user ${msg.author.username} ; (ID (${message.author.id})\`\`\``);
 msg.channel.sendEmbed(embed)
    ...
    if(!args[0] || isNaN(args[0])) return msg.channel.send('**lease Select Wallpaper Number.**');
...
fs.readFile(__dirname + `/${wallpapers[args[0]].src}`, function (err, Background) {
  ...
  msg.channel.send({file:canvas.toBuffer()})
    ...
    .setFooter(`Tip: To preview wallpaper try \`${prefix}set\``)
...
msg.channel.send("<a:6_:624217374241062922> |`-`__**Profile **__`-` | <a:6_:624217374241062922>" , {
